<template>
	<div :id="id" :class="classes">
		<label v-if="labelName && labelName !== ''" class="se-daterangepicker__label">{{ labelName }}</label>
		<date-range-picker
			ref="picker"
			v-model="dateRange"
            :singleDatePicker="typePicker"
            :autoApply="isAutoApply"
            :appendToBody="isAppendToBody"
        ...
        >
        	<template #input="picker">
        		<div class="se-daterangepicker__input">
	    			<span v-if="isDateRangeEmpty()" class="se-daterangepicker__placeholder">Select the date</span>
	    			<span v-else class="se-daterangepicker__value">{{ dateRange.startDate }} - {{ dateRange.endDate }}</span>
        		</div>
        		<i 
        ...
        <div v-if="info && info !== ''" class="se-daterangepicker__info">{{ info }}</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
		    ...
		    dateFormat (classes, date) {
				...
				let d1 = dateUtil.format(date, this.formatDate)
				let d2 = dateUtil.format(yesterday.setDate(yesterday.getDate() - 1), this.formatDate)
